...
.config(function($stateProvider) {
	...
	.state('xq', {
		...
		templateUrl:'src/views/xq/xq.html'
	})
	.state('xqContent', {
		url:'/xqContent',
		templateUrl:'src/views/xq/xqContent/xqContent.html'
	})
	.state('xqTotal', {
		url:'/xqTotal',
		templateUrl:'src/views/xq/xqTotal/xqTotal.html'
	})
...
.controller('xqCtrl',function($scope,$http,$stateParams) {
		...
		if(objData) {
			...
			getData = JSON.parse(localStorage.getItem('localStorageData'))
		}

		console.log(getData)

		$scope.nData = getData
		$scope.tag = getData.tags
		$scope.tabC = getData.table_of_contents
	...
	return {
		...
		link:function ($scope, $element, $attrs) {
			 ...
			 for(var i = 0;i < floInt;i++) {
			 	cnt++;
          		$(ele).append($('<img src="src/views/xq/imgs/star1.png">'))
			 }
			 if (upInt>floInt) {
           		cnt++;
                $(ele).append($('<img src="src/views/xq/imgs/star4.png">'))
             };
             for(var j = cnt; j<5;j++){
                cnt++;
                $(ele).append($('<img src="src/views/xq/imgs/star2.png">'))
             }
		}
